...
class AdministratorEventController {
    // Create event (administrator can create events)
    static async createEvent(req, res) {
        try {
            ...
            logger.error('Error in administrator createEvent:', error.message);
            return errorResponse(res, 500, 'Internal server error', error.message);
        }
    }

    // Get administrator's own events
    static async getMyEvents(req, res) {
        try {
            ...
            if (!result.success) {
                logger.error('Failed to fetch administrator events:', result.error);
    ...
    // Get upcoming events (administrator view)
    ...
    // Get past events (administrator view)
    ...
    // Update administrator's own event
    static async updateEvent(req, res) {
        try {
            ...
            // Check if event exists and administrator owns it
            ...
            logger.error('Error in administrator updateEvent:', error.message);
            return errorResponse(res, 500, 'Internal server error', error.message);
        }
    }

    // Delete administrator's own event
    static async deleteEvent(req, res) {
        try {
            ...
            logger.error('Error in administrator deleteEvent:', error.message);
            return errorResponse(res, 500, 'Internal server error', error.message);
        }
    }

    // Get event by ID (administrator can view any event)
    static async getEventById(req, res) {
        try {
            ...
            logger.error('Error in administrator getEventById:', error.message);
            return errorResponse(res, 500, 'Internal server error', error.message);
        }
    }

    // Search events (administrator can search all events)
    static async searchEvents(req, res) {
        try {
            ...
            logger.error('Error in administrator searchEvents:', error.message);
            return errorResponse(res, 500, 'Internal server error', error.message);
        }
    }

    // Get event statistics for administrator's events
    static async getAdministratorEventStats(req, res) {
        try {
            ...
            if (id) {
                ...
                if (!eventCheck.success || eventCheck.data.created_by !== adminId) {
                    return errorResponse(res, 403, 'Unauthorized to view stats for this event');
                }
                result = await Event.getEventStats(eventId);
            } else {
                // Get overall stats for events created by this administrator
                const allEventsResult = await Event.findAll(1, 99999, { created_by: adminId });
            ...
            if (!result.success) {
                logger.error('Failed to fetch administrator event statistics:', result.error);
            ...
            logger.error('Error in getAdministratorEventStats:', error.message);
            return errorResponse(res, 500, 'Internal server error', error.message);
        }
    }

    // Get event participants for administrator's events
    static async getEventParticipants(req, res) {
        try {
            ...
            // Check if administrator owns this event
            const eventCheck = await Event.findById(eventId);
            ...
            logger.error('Error in administrator getEventParticipants:', error.message);
            return errorResponse(res, 500, 'Internal server error', error.message);
        }
    }
}

module.exports = AdministratorEventController;
