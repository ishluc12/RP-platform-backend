...
-- ===============================================
-- This script fixes all database schema issues for appointments and availability

-- First, let's check what columns actually exist and fix them
DO $$ BEGIN -- Fix appointments table - add missing columns
IF NOT EXISTS (
    SELECT 1
    FROM information_schema.columns
    WHERE table_name = 'appointments'
      AND column_name = 'appointment_date'
) THEN -- Add appointment_date column
...
) THEN -- Migrate data from appointment_time to appointment_date
UPDATE appointments
...
DO $$ BEGIN -- Add availability_type column
IF NOT EXISTS (
    SELECT 1
    FROM information_schema.columns
...
SELECT 'staff_availability' as table_name,
    column_name,
    data_type,
    is_nullable,
    column_default
